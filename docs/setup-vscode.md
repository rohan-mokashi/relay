...
support and `.vscode/mcp.json`; the matching root `.mcp.json` preserves portable Agent Host
compatibility. No Relay-specific extension is required.
...
in either MCP configuration or a Relay capsule.
...
3. Select `relay`, choose **Start Server**, and review the one-time trust prompt. Trust it only if
   the command matches the checked-in `.vscode/mcp.json` and root `.mcp.json`.
