...
export class RelayService {
  ...
  private async principal(context: RequestContext) {
    return await this.repository.ensurePrincipal(context.principalRef, this.clock());
  }

  private async requireProject(principalId: string, projectId: string): Promise<Project> {
    const project = await this.repository.getProjectById(principalId, projectId);
    if (!project) throw new RelayError("NOT_FOUND", "Project was not found.");
    return project;
  }

  private async resolveProject(principalId: string, input: GetProjectInput): Promise<Project> {
    const project = input.project_id
      ? await this.repository.getProjectById(principalId, input.project_id)
      : await this.repository.getProjectBySlug(principalId, normalizeSlug(input.slug ?? ""));
    if (!project) throw new RelayError("NOT_FOUND", "Project was not found.");
    return project;
  }
  ...
  private async prepareArtifacts(
    ...
    if (existingIds.length > 0) {
      const existing = await this.repository.getArtifactsByIds(principalId, projectId, existingIds);
    ...
    const created = await this.repository.createArtifacts(
      principalId,
      projectId,
  ...
  async upsertProject(context: RequestContext, input: UpsertProjectInput) {
    return await this.repository.withRequest(async () => {
      ...
      return await this.repository.withIdempotency(
        principal.id,
        "upsert_project",
        ...
        now,
        async () => {
          const result = await this.repository.upsertProject(
            principal.id,
            normalizedInput,
            this.idFactory("prj"),
            now,
  ...
  async listProjects(context: RequestContext, input: ListProjectsInput) {
    return await this.repository.withRequest(async () => {
      const principal = await this.principal(context);
      return await this.repository.listProjects(principal.id, input);
    });
  }

  async getProject(context: RequestContext, input: GetProjectInput) {
    return await this.repository.withRequest(async () => {
      const principal = await this.principal(context);
      const project = await this.resolveProject(principal.id, input);
      const ids = await this.repository.getProjectRecordIds(principal.id, project.id);
      return {
        project,
  ...
  async createHandoff(context: RequestContext, input: CreateHandoffInput) {
    return await this.repository.withRequest(async () => {
      const principal = await this.principal(context);
      assertNoObviousSecrets(input);
      const payload = withoutIdempotencyKey(input);
      ...
      return await this.repository.withIdempotency(
        ...
        now,
        async () => {
          await this.requireProject(principal.id, input.project_id);
          if (input.supersedes_handoff_id) {
            const superseded = await this.repository.getHandoffById(
              principal.id,
          ...
          const artifactIds = await this.prepareArtifacts(
            principal.id,
            input.project_id,
            input.artifacts,
            input.artifact_refs,
          ...
          const handoff = await this.repository.insertHandoff(principal.id, {
            ...payload,
  ...
  ): Promise<{ handoff: Handoff; content_notice: string }> {
    return await this.repository.withRequest(async () => {
      const principal = await this.principal(context);
      const handoff = input.handoff_id
        ? await this.repository.getHandoffById(principal.id, input.handoff_id)
        : await this.repository.getLatestHandoff(principal.id, input.project_id ?? "");
      if (!handoff) throw new RelayError("NOT_FOUND", "Handoff was not found.");
      return { handoff, content_notice: CONTENT_NOTICE };
    });
  }

  async createCheckpoint(context: RequestContext, input: CreateCheckpointInput) {
    return await this.repository.withRequest(async () => {
      const principal = await this.principal(context);
      assertNoObviousSecrets(input);
      const payload = withoutIdempotencyKey(input);
      ...
      return await this.repository.withIdempotency(
        ...
        async () => {
          ...
          if (input.supersedes_checkpoint_id) {
            const superseded = await this.repository.getCheckpointById(
              principal.id,
          ...
          const checkpoint = await this.repository.insertCheckpoint(principal.id, {
            ...payload,
  ...
  async getLatestCheckpoint(context: RequestContext, input: GetLatestCheckpointInput) {
    return await this.repository.withRequest(async () => {
      const principal = await this.principal(context);
      await this.requireProject(principal.id, input.project_id);
  ...
  async getProjectContext(context: RequestContext, input: GetProjectContextInput) {
    return await this.repository.withRequest(async () => {
      const principal = await this.principal(context);
      const project = await this.resolveProject(principal.id, input);
      const handoff = await this.repository.getLatestHandoff(principal.id, project.id);
      const checkpoint = await this.repository.getLatestCheckpoint(principal.id, project.id);
      const artifacts = await this.repository.getProjectArtifacts(
        principal.id,
        project.id,
        input.artifact_limit,
      );
      const counts = await this.repository.countProjectRecords(principal.id, project.id);
      ...
      const latestCheckpoint = checkpoint
        ? {
            ...
            changed_files: compact
              ? checkpoint.changed_files.slice(0, 10)
      ...
      return {
        ...
        recommended_next_action:
          latestCheckpoint?.recommended_next_action ??
  ...
  async listProjectHistory(context: RequestContext, input: ListProjectHistoryInput) {
    return await this.repository.withRequest(async () => {
      const principal = await this.principal(context);
      await this.requireProject(principal.id, input.project_id);
      const page = await this.repository.listProjectHistory(principal.id, input);
      return {
        records: page.items,
  ...
  async getArtifactsForTesting(
    ...
    return await this.repository.withRequest(async () => {
      const principal = await this.principal(context);
      return await this.repository.getArtifactsByIds(principal.id, projectId, artifactIds);
    });
  }
}
